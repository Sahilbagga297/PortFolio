  ...
  return (
    <nav className="fixed top-0 w-full bg-transparent backdrop-blur-md px-6 py-6 z-50 shadow-lg rounded-3xl">
      <div className="max-w-7xl mx-auto">
        <div
          className={`bg-gray-900/60 backdrop-blur-lg border border-gray-700/50 rounded-2xl px-8 py-4 shadow-xl shadow-gray-900/20 transition-all duration-1000
          }`}
        >
            ...
            <div className="hidden md:flex space-x-8">
              {navItems.map((item) => (
                <NavLink
                  ...
                  to={item.path}
                  className={({ isActive }) =>
                    `text-gray-400 hover:text-white transition-all duration-300 font-medium relative group ${isActive ? "text-white" : ""
                    }`
                  }
                >
                  {item.name}
              ...
              {navItems.map((item) => (
                <li key={item.name}>
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `block text-gray-400 hover:text-white transition-all duration-300 font-medium ${isActive ? "text-white" : ""
                      }`
                    }
                  >
                    {item.name}
